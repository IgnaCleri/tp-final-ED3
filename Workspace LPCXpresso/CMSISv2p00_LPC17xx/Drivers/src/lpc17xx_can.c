...
/********************************************************************/ /**
                                                                        * @brief        Initialize
                                                                        * CAN peripheral with given
                                                                        * baudrate
                                                                        * @param[in]    CANx pointer
                                                                        * to LPC_CAN_TypeDef, should
                                                                        * be:
                                                                        *                 -
                                                                        * LPC_CAN1: CAN1 peripheral
                                                                        *                 -
...
/********************************************************************/ /**
                                                                        * @brief        Setup
                                                                        * Acceptance Filter Look-Up
                                                                        * Table
                                                                        * @param[in]    CANAFx
                                                                        * pointer to
                                                                        * LPC_CANAF_TypeDef Should
                                                                        * be: LPC_CANAF
                                                                        * @param[in]    AFSection
                                                                        * the pointer to
                                                                        * AF_SectionDef structure It
                                                                        * contain information about
                                                                        * 5 sections will be install
                                                                        * in AFLUT
                                                                        * @return         CAN Error
                                                                        *   could be:
                                                                        *                 -
                                                                        * CAN_OBJECTS_FULL_ERROR: No
                                                                        * more rx or tx objects
                                                                        * available
                                                                        *                 -
...
CAN_ERROR CAN_SetupAFLUT(LPC_CANAF_TypeDef *CANAFx, AF_SectionDef *AFSection) {
    ...
    if (AFSection->FullCAN_Sec == NULL) {
        ...
        for (i = 0; i < (AFSection->FC_NumEntry); i++) {
            ...
            AFSection->FullCAN_Sec =
                (FullCAN_Entry *)((uint32_t)(AFSection->FullCAN_Sec) + sizeof(FullCAN_Entry));
        }
    }
    ...
    if (AFSection->SFF_GPR_Sec != NULL) {
        for (i = 0; i < (AFSection->SFF_GPR_NumEntry); i++) {
            ...
            entry = (ctrl1 << 29) | (dis1 << 28) | (lowerSID << 16) | (ctrl2 << 13) | (dis2 << 12) |
                    (upperSID << 0);
            LPC_CANAF_RAM->mask[count] = entry;
            CANAF_gstd_cnt++;
            count++;
            AFSection->SFF_GPR_Sec =
                (SFF_GPR_Entry *)((uint32_t)(AFSection->SFF_GPR_Sec) + sizeof(SFF_GPR_Entry));
        }
    }
    ...
    if (AFSection->EFF_GPR_Sec != NULL) {
        for (i = 0; i < (AFSection->EFF_GPR_NumEntry); i++) {
            ...
            AFSection->EFF_GPR_Sec =
                (EFF_GPR_Entry *)((uint32_t)(AFSection->EFF_GPR_Sec) + sizeof(EFF_GPR_Entry));
        }
    }
...
/********************************************************************/ /**
                                                                        ...
                                                                        * LPC_CAN2: CAN2 peripheral
                                                                        * @param[in]    id: The ID
                                                                        * of entry will be added
                                                                        * @param[in]    format: is
                                                                        * the type of ID Frame
                                                                        * Format, should be:
                                                                        *                 -
                                                                        * STD_ID_FORMAT: 11-bit ID
                                                                        * value
                                                                        *                 -
                                                                        * EXT_ID_FORMAT: 29-bit ID
                                                                        * value
                                                                        * @return         CAN Error,
                                                                        * could be:
                                                                        *                 -
...
CAN_ERROR CAN_LoadFullCANEntry(LPC_CAN_TypeDef *CANx, uint16_t id) {
    ...
    LPC_CANAF_RAM->mask[cnt1] = LPC_CANAF_RAM->mask[cnt1 + 1] = LPC_CANAF_RAM->mask[cnt1 + 2] =
        0x00;
    cnt1 += 3;
    while (bound1--) {
...
/********************************************************************/ /**
                                                                        ...
                                                                        * LPC_CAN2: CAN2 peripheral
                                                                        * @param[in]    lowerID,
                                                                        * upperID: lower and upper
                                                                        * identifier of entry
                                                                        * @param[in]    format: type
                                                                        * of ID format, should be:
                                                                        *                 -
                                                                        ...
                                                                        * more space available
                                                                        *********************************************************************/
CAN_ERROR CAN_LoadGroupEntry(LPC_CAN_TypeDef *CANx, uint32_t lowerID, uint32_t upperID,
...
/********************************************************************/ /**
                                                                        * @brief        Remove AFLUT
                                                                        * entry (FullCAN entry and
                                                                        * Explicit Standard entry)
                                                                        * @param[in]    EntryType:
                                                                        * the type of entry that
...
CAN_ERROR CAN_RemoveEntry(AFLUT_ENTRY_Type EntryType, uint16_t position) {
    ...
    Acceptance Filter Mode Register = Off */
    LPC_CANAF->AFMR = 0x00000001;
    total = ((CANAF_FullCAN_cnt + 1) >> 1) + ((CANAF_std_cnt + 1) >> 1) + CANAF_gstd_cnt +
            CANAF_ext_cnt + (CANAF_gext_cnt << 1);
    ...
    /************** Remove Explicit Extended ID Entry *************/
        ...
        if ((CANAF_ext_cnt == 0) || (position >= CANAF_ext_cnt)) {
            ...
            cnt = ((CANAF_FullCAN_cnt + 1) >> 1) + ((CANAF_std_cnt + 1) >> 1) + CANAF_gstd_cnt +
                  position + 1;
            bound = total + CANAF_FullCAN_cnt * 3;
            while (cnt < bound) {
                LPC_CANAF_RAM->mask[cnt - 1] = LPC_CANAF_RAM->mask[cnt];
...
/********************************************************************/ /**
                                                                        ...
                                                                        * LPC_CAN2: CAN2 peripheral
                                                                        * @param[in]    CAN_Msg
                                                                        * point to the CAN_MSG_Type
                                                                        * Structure, it contains
                                                                        * message information such
                                                                        * as: ID, DLC, RTR, ID
                                                                        * Format
                                                                        * @return         Status:
                                                                        *                 - SUCCESS:
...
/********************************************************************/ /**
                                                                        ...
                                                                        * FullCAN Object is not be
                                                                        * received
                                                                        *                 - CAN_OK:
                                                                        * Received FullCAN Object
                                                                        * successful
                                                                        *
                                                                        *********************************************************************/
...
/********************************************************************/ /**
                                                                        ...
                                                                        *                 -
                                                                        * LPC_CAN2: CAN2 peripheral
                                                                        * @param[in]    arg: type of
                                                                        * CAN status to get from CAN
                                                                        * status register Should be:
                                                                        *                 -
                                                                        * CANCTRL_GLOBAL_STS: CAN
                                                                        * Global Status
                                                                        *                 -
                                                                        * CANCTRL_INT_CAP: CAN
...
/********************************************************************/ /**
                                                                        * @brief        Get CAN
                                                                        * Central Status
                                                                        * @param[in]    CANCRx point
                                                                        * to LPC_CANCR_TypeDef,
                                                                        * should be: LPC_CANCR
                                                                        * @param[in]    arg: type of
                                                                        * CAN status to get from CAN
                                                                        ...
                                                                        *                 Should be:
                                                                        *                 -
                                                                        * CANCR_TX_STS: Central CAN
                                                                        * Tx Status
                                                                        *                 -
                                                                        * CANCR_RX_STS: Central CAN
                                                                        * Rx Status
                                                                        *                 -
                                                                        * CANCR_MS: Central CAN
                                                                        * Miscellaneous Status
                                                                        * @return         Current
                                                                        * Central Status that you
                                                                        * want to get value
                                                                        *********************************************************************/
...
/********************************************************************/ /**
                                                                        ...
                                                                        * Interrupt Enable
                                                                        *                 -
                                                                        * CANINT_EIE: CAN Error
                                                                        * Warning Interrupt Enable
                                                                        *                 -
                                                                        * CANINT_DOIE: CAN Data
                                                                        * Overrun Interrupt Enable
                                                                        *                 -
                                                                        * CANINT_WUIE: CAN Wake-Up
                                                                        * Interrupt Enable
                                                                        *                 -
                                                                        * CANINT_EPIE: CAN Error
                                                                        * Passive Interrupt Enable
                                                                        *                 -
                                                                        * CANINT_ALIE: CAN
...
/********************************************************************/ /**
                                                                        ...
                                                                        * @param[in]    CANAFx point
                                                                        * to LPC_CANAF_TypeDef
                                                                        * object, should be:
                                                                        * LPC_CANAF
                                                                        * @param[in]    AFMode: type
                                                                        * of AF mode that you want
                                                                        * to set, should be:
                                                                        *                 -
                                                                        * CAN_Normal: Normal mode
                                                                        *                 -
                                                                        * CAN_AccOff: Acceptance
                                                                        * Filter Off Mode
                                                                        *                 -
                                                                        * CAN_AccBP: Acceptance
                                                                        * Fileter Bypass Mode
                                                                        *                 -
...
/********************************************************************/ /**
                                                                        ...
                                                                        * LPC_CAN2: CAN2 peripheral
                                                                        * @param[in]    mode: type
                                                                        * of CAN mode that you want
                                                                        * to enable/disable, should
                                                                        * be:
                                                                        *                 -
...
/*********************************************************************/ /**
                                                                         ...
                                                                         * Reception request
                                                                         *                 -
                                                                         * CAN_CMR_STB1: Select Tx
                                                                         * Buffer 1 request
                                                                         *                 -
                                                                         * CAN_CMR_STB2: Select Tx
                                                                         * Buffer 2 request
                                                                         *                 -
                                                                         * CAN_CMR_STB3: Select Tx
                                                                         * Buffer 3 request
                                                                         * @return        CANICR
                                                                         * (CAN interrupt and
...
/*********************************************************************/ /**
                                                                         ...
                                                                         * @param[in]    CANAFx
                                                                         * point to
                                                                         * LPC_CANAF_TypeDef object,
                                                                         * should be: LPC_CANAF
                                                                         * @return        IntStatus,
                                                                         * could be:
                                                                         *                 - SET: if
                                                                         * FullCAN interrupt is
                                                                         * enable
                                                                         *                 - RESET:
                                                                         * if FullCAN interrupt is
                                                                         * disable
                                                                         **********************************************************************/
